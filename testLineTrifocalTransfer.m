...
pose_used = pose_num - 2;
for i = 1 : pose_used
    Rwc{i} = Rwc{i} * rodrigues(0.2 * (rand(3,1) - 0.5));
...
for iter = 1 : 20
    ...
    fprintf(sprintf('iter: %d, err:%f, res_count: %d, rou12 - gt = [%f %f], rank(H): %f, H size: %f\n', iter, err_sum, res_count, max(abs(rou1 - rou1_gt)), max(abs(rou2 - rou2_gt)), rank(H), size(H,1)));
...
d_nt_d_Rth_left = -SkewSymMat(Rth * nh) - SkewSymMat(tth) * SkewSymMat(Rth * vh) + SkewSymMat(Rth * vh) * SkewSymMat(tth);
d_nt_d_tth_left = -SkewSymMat(Rth * vh);
d_vt_d_Rth_left = -SkewSymMat(Rth * vh);
d_vt_d_tth_left = zeros(3,3);
d_line_t_d_Tth_left = [d_nt_d_Rth_left d_nt_d_tth_left;...
    d_vt_d_Rth_left d_vt_d_tth_left];



d_nt_d_Rth_right = -Rth * SkewSymMat(nh) - SkewSymMat(tth) * Rth * SkewSymMat(vh);
d_nt_d_tth_right = -SkewSymMat(Rth * vh) * Rth;
d_vt_d_Rth_right = -Rth * SkewSymMat(vh);
d_vt_d_tth_right = zeros(3,3);
d_line_t_d_Tth_right = [d_nt_d_Rth_right d_nt_d_tth_right;...
    d_vt_d_Rth_right d_vt_d_tth_right];

d_line_t_d_Tth_right_check = d_line_t_d_Tth_left * Adj([Rth tth;0 0 0 1]);
jac_diff = d_line_t_d_Tth_right - d_line_t_d_Tth_right_check;

if ~right_perturb
    d_line_t_d_Tth = d_line_t_d_Tth_left;
else
    d_line_t_d_Tth = d_line_t_d_Tth_right;
end
